document.addEventListener("DOMContentLoaded", function () {
  ...
  const haciendas = {
    1: [
      "Rio Frayle", "La Industria", "Salsipuedes", "Pergola II", "Pergola III", "ACUARIO", "PERGOLA I",
      "Vallecito", "Papayal", "La Linda", "El Rincon", "Papayalito", "El Tablon Bajo", "Cantarohondo",
      "El Tablon Alto", "El Baranco", "Santa Barbara Participacion", "La Michela",
    ],
    2: [
      ...
      "Hato", "Arado", "Yarumales", "Mendiola La Paila", "Benitez", "Hato San Carlos", "Mundo Cabaña",
      "La Munda Esmeralda", "Canada Cabaña", "Hato Agrocañas", "Hato Cavi",
    ],
    3: [
      ...
      "Cabaña Rabinovich", "Naranjo Agrocañas", "Naranjo", "Muchilanga", "Taula Ana", "Naranjo Esmeralda",
      "Gobierno Negro", "Granada", "Cabaña Agrocañas", "Naranjo Cabaña", "Muchilanga Cabaña",
      "Vegas Cabaña", "Alvaro Naranjo Poveda Arrey", "El Rebaño", "Dulce Jugada Participacion",
    ],
    4: [
      ...
      "La Loma", "Corcovao Adecañas", "Veracruz Adecañas", "Sarria", "Nilo Toro", "Janeiro Esmeralda",
      "Hermanas Florez", "Las Cañas", "Ana Maria Bonilla", "El Cairo", "Tablanca", "Holanda", "La Camelia",
      "Sucursal", "El Socorro Cabaña", "Monaco", "La Alsacia", "Ventura Olgin",
    ],
    5: [
      "Casablanca", "Bocas de Palo", "Perico Negro", "El Lago", "Victoria", "Curazao", "Coburgo Cantarito",
      "Bohemia", "Curazao Escobar", "Ceibal Nereida Sepulveda Roja", "Curazao Agroindustrias",
      "Curazao Velez", "La Moraleja", "Curazao Esmeralda", "Lago Rabinovich", "Quintero", "Sofia", "Sarria",
      "Victoria Esmeralda", "Avenida", "Sarria Agrocañas", "La Moraleja Accion Fiduciaria",
      "Campo Alegre Argos Arrendo", "Moraleja Arango", "Cachipay", "Moraleja PYS Proyectos Soll",
    ],
  };
  ...
  // Genera las opciones para el select de números y letras
  ...
  // Actualiza el campo de suerte con el número y la letra seleccionados
  const suerteInput = document.getElementById("suerte");

  const updateSuerte = () => {
    const numeroSuerte = numeroSuerteSelect.value;
    const letraSuerte = letraSuerteSelect.value;
    if (numeroSuerte && letraSuerte) {
      suerteInput.value = `${numeroSuerte} - ${letraSuerte}`;
    } else {
      suerteInput.value = "";
    }
  };

  // Agregar eventos de cambio para actualizar el valor de "Suerte"
  numeroSuerteSelect.addEventListener("change", updateSuerte);
  letraSuerteSelect.addEventListener("change", updateSuerte);

  // Función para enviar el formulario y redirigir a la página anterior
  document.getElementById('fuel-supply-form').addEventListener('submit', function(event) {
  ...
  document.getElementById('btn-salir').addEventListener('click', function() {
    window.location.href = 'otra_pagina.html'; // Cambia 'otra_pagina.html' por la URL a la que quieras redirigir
  });
});
